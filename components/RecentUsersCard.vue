  ...
  <main>
    <div v-if="!loading && usersList" class="rounded-2xl bg-white p-6 border-gray-50 border space-y-6">
      ...
      <div v-if="activeTable === 'reserved_usernames'" class="overflow-x-auto">
        <table v-if="usersList.length && !loading" class="min-w-full divide-y-2 divide-gray-200 bg-white text-sm">
          <tbody class="divide-y divide-gray-200">
            <tr class="h-14" v-for="(itm, idx) in computedUsersList" :key="idx">
              <td class="whitespace-nowrap px-4 font-medium text-gray-900 bg-white">
                 <div v-if="itm.profilePicture">
                  <CoreImageZoom  class="cursor-pointer h-10 w-10" :src="itm.profilePicture" />
                 </div>
                <p v-else
                  class="rounded-full font-semibold h-10 w-10 bg-gray-300 text-gray-800 flex justify-center items-center">
      ...
      <div v-if="activeTable === 'recent_signup'" class="overflow-x-auto">
        <table v-if="usersList.length && !loading" class="min-w-full divide-y-2 divide-gray-200 bg-white text-sm">
          <tbody class="divide-y divide-gray-200">
            <tr class="h-14" v-for="(itm, idx) in recentSignups" :key="idx">
              <td class="whitespace-nowrap px-4  font-medium text-gray-900 bg-white">
                 <div v-if="itm.profilePicture">
                  <CoreImageZoom  class="w-10 h-10 cursor-pointer" :src="itm.profilePicture" />
                 </div>
                <p v-else
                  class="rounded-full w-10 h-10 font-semibold bg-gray-300 text-gray-800 flex justify-center items-center">
                  {{ generateInitials(itm.fname, itm.lname) }}</p>
              </td>
              <td class="whitespace-nowrap text-base md:text-sm px-4 text-gray-700">
                <nuxt-link class="text-blue-700 no-underline hover:underline" :to="`/dashboard/users/${itm.id}`">{{
...
const computedUsersList = computed(() => {
  return usersList.value.slice(0, 10)
})
